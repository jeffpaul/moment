...
| Destination visibility | Only **connected** connectors are offered as publish destinations; auto-applied type defaults filter to connected too (explicit API selections honored as-is). AI Assist UI only renders when a provider `is_available()`. | A destination that can't publish or return replies shouldn't be offered. Demo/test escape hatch: `add_filter( 'moment_show_unconnected_connectors', '__return_true' )` (the E2E suite and CI use this via a mu-plugin). Model defaults stay recorded in `_moment_default_destinations`. |
